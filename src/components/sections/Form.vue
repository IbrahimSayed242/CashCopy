<template>
<!-- component -->
<div class="flex justify-center items-center w-screen h-screen bg-green-100">
	<!-- COMPONENT CODE -->
	<div class="container mx-auto my-4 px-4 lg:px-20">

		<form @submit.prevent="handleSubmit" class="w-full p-8 my-4 md:px-12 lg:w-9/12 lg:pl-20 lg:pr-40 mr-auto rounded-2xl shadow-2xl">
			<div class="flex">
				<h1 class="font-bold uppercase text-5xl">Send us a <br /> message</h1>
			</div>
			<div class="grid grid-cols-1 gap-5 md:grid-cols-2 mt-5">
				<input v-model="formData.full_name" class="w-full bg-gray-100 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
            type="text" placeholder="First Name*"   />
				<input class="w-full bg-gray-100 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
            type="text" placeholder="Last Name*" />
				<input v-model="formData.email" class="w-full bg-gray-100 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
            type="email" placeholder="Email*" />
				<input v-model="formData.phone"  class="w-full bg-gray-100 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
            type="number" placeholder="Phone*" />
				...
				<div class="my-2 w-1/2 lg:w-1/4">
					<button class="uppercase text-sm font-bold tracking-wide bg-blue-900 text-gray-100 p-3 rounded-lg w-full 
                      focus:outline-none focus:shadow-outline" @click="handleSubmit">
            Send Message
          </button>
				</div>
			</form>
...
<div class="flex items-end justify-end fixed bottom-0 right-0 mb-4 mr-4 z-10">
  <div>
    <a title="Buy me a pizza" href="https://www.buymeacoffee.com/Dekartmc" target="_blank" class="block w-16 h-16 rounded-full transition-all shadow hover:shadow-lg transform hover:scale-110 hover:rotate-12">
      <img class="object-cover object-center w-full h-full rounded-full" src="/images/bitcoin-cash.jpg"/>
    </a>
  </div>
</div>
...
export default {
  ...
  methods: {
    async handleSubmit() {
      try {
        const frappeApiUrl = 'https://alltargeting.com/api/method/heero.api.flagedu_lead';

        const data = {
          email: this.formData.email,
          name1: this.formData.full_name,
          phone: this.formData.phone,
          interest: 'Flagedu Contact Us',
          message: this.formData.message,
          afp: 'Flagedu-Contact',
          // Add other relevant data to send to the Frappe API
        };
        ...
        if (response.ok) {
          console.log('POST request to Frappe API successful.');
        } else {
          console.error('POST request to Frappe API failed. Status code:', response.status);
        }
      } catch (error) {
